...
class KnowledgeBase:
    ...
    #**** se comenta para cambiar a version con chunks exclusivos po Banco para mejoara precision de RAG.
...
#************* todos los vectores en REDIS cuando hemos cambiado los archivos de rag **************************
    ...
    def load_from_folder(self, folder_path: str = "rag", force_rebuild: bool = False):
        ...
        # 1) Leer y fragmentar por archivo (NO mezclar todo)
        files = [f for f in os.listdir(folder_path) if f.lower().endswith((".txt", ".md"))]
        if not files:
            print(" No hay archivos .txt o .md para procesar.")
            return
        ...
        def infer_bank(file_name: str) -> str:
            fn = file_name.lower()
            if "santander" in fn:
                return "santander"
            if "banco_de_chile" in fn or "banco de chile" in fn or "chile" in fn:
                return "banco_de_chile"
            if "estado" in fn:
                return "banco_estado"
            if "bci" in fn:
                return "bci"
            if "itau" in fn or "itaú" in fn:
                return "itau"
            if "scotiabank" in fn:
                return "scotiabank"
            if "falabella" in fn:
                return "falabella"
            if "ripley" in fn or "Ripley" in fn:
                return "ripley"
        ...
        for file_name in sorted(files):
            ...
            print(f"Leído y fragmentado: {file_name} ({len(chunks)} chunks) -> bank={bank}")

        # 2) Crear índice (ya estás en modo rebuild)
        ...
        # 4) Guardar en Redis con metadata
        pipe = self.redis_client.pipeline()
        for i, (text, embedding, meta) in enumerate(zip(texts, embeddings, metas)):
     ...
     #****************** 06: FEBRERO ---> ANTIGUO CODIGO, NO HACE CHUNKS POR BANCO ************
    ...
    def find_vector_in_redis(self, query: str, k: int = 3, bank : str | None= None) -> str:
        ...
        base = f"@bank:{{{bank}}}" if bank else "*"

        q = (
            Query(f"{base}=>[KNN {k} @{self.vector_field_name} $vec_param AS vector_score]")
            .sort_by("vector_score")
            .paging(0, k)
            .return_fields("text", "vector_score", "bank", "source")
            .dialect(2)
        )
